  ...
  return (
    <div className='min-w-[230px] min-h-[402px] rounded-2xl flex flex-col justify-between relative cursor-pointer' onClick={clickhandler}>
        ...
        <p className='text-white text-ellipsis mt-3 text-[20px] '>
          {(movie.title.length > 20) ? movie.title.substring(0,20)+"..." : movie.title}</p>
        <p className='text-white opacity-50 text-[14px]'>{movie.release_date}</p>
    </div>
  )
}
